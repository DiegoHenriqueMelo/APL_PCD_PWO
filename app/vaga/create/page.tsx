...
export default function CriarVaga() {
  ...
  const [formData, setFormData] = useState({
    ...
    acessibilidades: [] as string[], // Array para múltiplas acessibilidades selecionadas
  });

  const [companyAcessibilidades, setCompanyAcessibilidades] = useState<string[]>([]);
  const [errors, setErrors] = useState<Record<string, string>>({});
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [loadingAcessibilidade, setLoadingAcessibilidade] = useState(true);
  ...
  // Carregar acessibilidades da empresa do contexto
  useEffect(() => {
    if (!isLoading && isAuthenticated && userType === 'company' && user) {
      try {
        setLoadingAcessibilidade(true);
        
        // Obter acessibilidades do contexto (salvas no login)
        const companyUser = user as any;
        const accessibility = companyUser.accessibility || '';
        
        // Dividir as acessibilidades por vírgula
        const acessibilidadesList = accessibility
          .split(',')
          .map((a: string) => a.trim())
          .filter((a: string) => a.length > 0);
        
        console.log('=== ACESSIBILIDADES DA EMPRESA ===');
        console.log('String:', accessibility);
        console.log('Array:', acessibilidadesList);
        
        setCompanyAcessibilidades(acessibilidadesList);
      } catch (error) {
        console.error('Erro ao processar acessibilidades:', error);
      } finally {
        setLoadingAcessibilidade(false);
      }
    }
  }, [user, isAuthenticated, userType, isLoading]);
  ...
  const handleAcessibilidadeChange = (value: string) => {
    setFormData((prev) => {
      const acessibilidades = prev.acessibilidades.includes(value)
        ? prev.acessibilidades.filter((a) => a !== value)
        : [...prev.acessibilidades, value];
      return { ...prev, acessibilidades };
    });

    // Limpar erro se existir
    if (errors.acessibilidades) {
      setErrors((prev) => ({
        ...prev,
        acessibilidades: '',
      }));
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.acessibilidades || formData.acessibilidades.length === 0) {
      newErrors.acessibilidades = 'Selecione pelo menos uma acessibilidade';
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const vagaData = {
        ...
        accessibility: formData.acessibilidades.join(', '), // Acessibilidades selecionadas
      };
          ...
          <div className="p-8 md:p-10">
            <form onSubmit={handleSubmit} className="space-y-8">
              ...
              <div>
                ...
                <div className="space-y-4">
                  <p className="text-sm text-gray-600">
                    Selecione as acessibilidades que esta vaga oferece (uma ou mais opções)
                  </p>
                  
                  {loadingAcessibilidade ? (
                  ...
                  ) : companyAcessibilidades.length === 0 ? (
                      ...
                      <div className="flex">
                        ...
                        <div className="ml-3">
                          <p className="text-sm text-yellow-700">
                            Sua empresa ainda não cadastrou acessibilidades. 
                            <a href="/perfil/editar" className="font-medium underline hover:text-yellow-800 ml-1">
                              Adicione acessibilidades ao seu perfil
                            </a>
                          </p>
                        </div>
                      </div>
                    ...
                    <div className="grid md:grid-cols-2 gap-3">
                      {companyAcessibilidades.map((acessibilidade, index) => (
                        <label
                          key={index}
                          className={`flex items-start p-4 border-2 rounded-lg cursor-pointer transition ${
                            formData.acessibilidades.includes(acessibilidade)
                              ? 'bg-orange-50'
                              : 'border-gray-300 hover:opacity-80 bg-white'
                          }`}
                          style={formData.acessibilidades.includes(acessibilidade) ? {borderColor: '#ECAE7D'} : {}}
                        >
                          <input
                            type="checkbox"
                            checked={formData.acessibilidades.includes(acessibilidade)}
