export const loginCompany = async (credentials: { email: string; senha: string }) => {

    console.log(credentials)
  try {
    const payload = JSON.stringify(credentials);
    console.log('=== LOGIN EMPRESA ===');
    console.log('URL:', 'http://localhost:3001/login/empresa');
    console.log('Payload enviado:', payload);
    
    const response = await fetch('http://localhost:3001/login/empresa', {
      method: 'POST',
      headers: {
